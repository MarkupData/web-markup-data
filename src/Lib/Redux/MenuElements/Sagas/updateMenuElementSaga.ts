...
function* updateMenuElementSaga(action: PayloadAction<ElementMenu>) {
	try {
		const response: Response = yield menuService.update(action.payload);
...
export default updateMenuElementSaga;
